...
contract SubscriptionsBoundAccountsManager is SuperAppBaseFlow, ERC20 {    
    ...
    address public outflowNFT;
    address public sbaImpl;
    ISuperToken public superCelo; // CELOx

    int96 public inflowTier1 = 31709791983;   // 1 CELOx / year
    int96 public inflowTier42 = 1331811263318; // 42 CELOx / year
    ...
    constructor() SuperAppBaseFlow(
        ...
        _owner = msg.sender;
        superCelo = ISuperToken(0x671425Ae1f272Bc6F79beC3ed5C4b00e9c628240);
        registry = IERC6551Registry(0x34e1408B9B047fE12a1B0C655B2D58775AcF579A);
        outflowNFT = 0xbe49ac1EadAc65dccf204D4Df81d650B50122aB2;
    ...
    // compute the outflowNFT tokenID from keccak of chainId, supertoken, sender and receiver
    ...
    // user must approve the contract to swap CELOx to credits
    function buyMoreCredits(address sba, uint256 amount) external {
        require(amount != 0, "InvalidAmount");
        
        superCelo.transferFrom(msg.sender, address(this), amount);
       
        _mint(sba, amount);
    }

    // allow users to spend some credits, test function
    ...
    {
        int96 inflowRate = superCelo.getFlowRate(sender, address(this));
    ...
    // Withdraw superCelo to the dapp owner
    function withdrawG(uint256 amount) external {
        require(superCelo.balanceOf(address(this)) >= amount);
        superCelo.transfer(_owner, amount);
    }
